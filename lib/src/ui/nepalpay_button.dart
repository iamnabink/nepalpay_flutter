...
class NepalPayButton extends StatelessWidget {
  /// button width
  final double? width;
  ...
  final Function(NepalPayResponse) onSuccess;
  ...
  final NepalPayConfig paymentConfig;
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      onPressed: () async {
        try {
          final result = await NepalPay.i
              .init(context: context, nepalPayConfig: paymentConfig);
          if (result.hasData) {
      ...
      child: widget ?? Text(title ?? 'Pay with NepalPay', style: textStyle),
    );
  }
}
